...
namespace OpsiClientSharp.RpcInterfaces
{
    public class DepotInterface : RpcInterface<DepotInterface>
    {
        ...
        public Task<string> GetMd5SumAsync(string absoluteFilePath) =>
            OpsiClient.ExecuteAsync<string>(new Request(GetFullMethodName("getMD5Sum")).AddParameter(absoluteFilePath));
        ...
        /// <param name="timeout">
        ///     The timeout to wait until the product is installed. We don't have any status update only if an error appears. Another solution would be to use the SSH
        ///     Command or check the logs
        /// </param>
        ...
        public Task InstallPackageAsync(string absoluteFilePath, int timeout = 180) =>
            OpsiClient.ExecuteAsync<string>(new Request(GetFullMethodName("installPackage")).AddParameter(absoluteFilePath), timeout);
        ...
        /// <param name="timeout">
        ///     The timeout to wait until the product is uninstalled. We don't have any status update only if an error appears. Another solution would be to use the SSH
        ///     Command or check the logs
        /// </param>
        /// <returns>Nothing if successful otherwise an exception</returns>
        public Task UninstallPackageAsync(string productId, int timeout = 180) =>
            OpsiClient.ExecuteAsync<string>(new Request(GetFullMethodName("uninstallPackage")).AddParameter(productId));
    }
}
